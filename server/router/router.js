...
module.exports = app => {
    ...
    app.post('/addCar', (req, res) => {
        ...
        // 如果数量为零也不行
        if (req.body.count == 0 || req.body.id == null) {
            res.send({ msg: "数量不能为零或者传递的id有误", status: 207 });
            return
        }
    ...
    app.post('/deleteCar', (req, res) => {
        ...
        if (!req.body.carIdArr instanceof Array && req.body.carIdArr.length == 0) {
            res.send({ msg: "传递的carIdArr不是一个数组或者是空数组,请检查", status: 209 });
            return
        }

        // // 验证传递的参数是否正确,否则返回参数不对  怕参数不对进行比较会产生一些后果
        if (!req.body.hasOwnProperty('id') || !req.body.hasOwnProperty('carIdArr')) {
            res.send({ msg: "传递的参数有误请检查", status: 206 });
            return
        }
        ...
        function writeJson(params) {
            ...
            fs.readFile('./data/car.json', function (err, data) {
                ...
                person.car = person.car.filter((item) => {
                    ...
                    if (!(item.id == params.id && params.carIdArr.indexOf(item.carId) > -1)) {
                        console.log('测试我们满不满足');
                        return item
                    }
                })
